...
mod lite_regex {
    ...
    impl RegexCompiler for LiteRegexCompiler {
        ...
        fn name(&self) -> &'static str { "regex-lite" }
    }
...
mod fancy_regex_adapter {
    ...
    impl RegexCompiler for FancyRegexCompiler {
        ...
        fn name(&self) -> &'static str { "fancy-regex" }
    }
...
mod regex_automata_adapter {
    ...
    impl RegexCompiler for RegexAutomataCompiler {
        ...
        fn name(&self) -> &'static str { "regex-automata" }
    }
                    ...
                    regex_syntax::ast::FlagsItemKind::Flag(flag) => {
                        match flag {
                            ...
                            regex_syntax::ast::Flag::DotMatchesNewLine => {
                                match options.library {
                                    "regex" | "regex-lite" | "regex-automata" 
                                    | "pcre2" | "onig" | "fancy-regex" => flag_str.push('s'),
                                    _ => return Err(format!("库 {} 不支持 's' 标志", options.library)),
                                }
                            }
                            regex_syntax::ast::Flag::SwapGreed => {
                                match options.library {
                                    "regex" | "regex-lite" | "regex-automata" 
                                    | "pcre2" | "onig" | "fancy-regex" => flag_str.push('U'),
                                    _ => return Err(format!("库 {} 不支持 'U' 标志", options.library)),
                                }
                            }
                            regex_syntax::ast::Flag::IgnoreWhitespace => {
                                match options.library {
                                    "regex" | "fancy-regex" | "pcre2" | "onig" => flag_str.push('x'),
                                    _ => return Err(format!("库 {} 不支持 'x' 标志", options.library)),
                                }
                            }
                            regex_syntax::ast::Flag::Unicode => {
                                if !options.supports_unicode_classes {
                                    return Err(format!("库 {} 不支持 'u' (Unicode) 标志", options.library));
                                }
                                flag_str.push('u');
                            }
                            _ => return Err(format!("库 {} 不支持标志 {:?}", options.library, flag)),
                        }
                    }
...
fn contains_named_backreference(ast: &Ast, name: &str) -> bool {
    match ast {
        ...
        Ast::Repetition(rep) => {
            contains_named_backreference(&rep.ast, name)
        }
        Ast::Group(group) => {
            contains_named_backreference(&group.ast, name)
        }
        _ => false,
    }
}
...
/// 翻译重复模式
fn translate_repetition(rep: &regex_syntax::ast::Repetition, inner: &str, options: &TranslatorOptions) -> String {
...
/// 翻译Unicode字符类
fn translate_unicode_class(class: &regex_syntax::ast::ClassUnicode, options: &TranslatorOptions) -> Result<String, String> {
    match &class.kind {
        regex_syntax::ast::ClassUnicodeKind::OneLetter(letter) => {
            match letter {
                ...
                _ => {
                    // 其他单个字母的Unicode字符类
                    if !options.supports_unicode_classes {
                        return Err(format!("库 {} 不支持Unicode字符类 \\{}", options.library, letter));
                    }
                    Ok(format!(r"\{}", letter))
                }
            }
...
/// 翻译Perl字符类
fn translate_perl_class(class: &regex_syntax::ast::ClassPerl, options: &TranslatorOptions) -> Result<String, String> {
    match &class.kind {
...
fn contains_unsupported_features(ast: &Ast) -> bool {
    ...
    match ast {
        ...
        Ast::Repetition(rep) => {
            contains_unsupported_features(&rep.ast)
        },
        Ast::Group(group) => {
            contains_unsupported_features(&group.ast)
        },
        _ => false,
    }
}
...
static REGEX_LIB_MANAGER: Lazy<RegexLibManager> = Lazy::new(|| {
    ...
    if let Ok(priority) = std::env::var("REGEX_BASELINE_PRIORITY") {
        let priority_list: Vec<&'static str> = priority.split(',')
            .map(|s| match s.trim() {
                "regex" => "regex",
...
fuzz_target!(|case: &[u8]| -> Corpus {
    ...
    // 检查是否包含连续的重复量词
    let has_consecutive_quantifiers = pattern_str.chars()
        .collect::<Vec<_>>()
        .windows(3)
        .any(|window| {
            window.iter().all(|&c| c == '*') || 
...
// Perl 类
if pattern_str.contains("\\d") || pattern_str.contains("\\D") ||
   pattern_str.contains("\\w") || pattern_str.contains("\\W") ||
   pattern_str.contains("\\s") || pattern_str.contains("\\S")
{
    return Corpus::Reject;
}
...
match compare_libraries_safely(&REGEX_LIB_MANAGER, pattern_str, &ast) {
    ...
    Err(ComparisonError::CompilationFailed(_)) => {
        Corpus::Reject
    }
    // 没找到基准库
