...
use egg::{
    define_language, rewrite, EGraph, Id, Language, RecExpr, Rewrite, Runner,
};
...
/// ```
pub fn pattern_to_egraph(
    pattern: &str,
) -> Result<(EGraph<RegexLang, ()>, Id), String> {
    // Parse the regex pattern into an AST
    let ast = Parser::new()
        .parse(pattern)
        .map_err(|e| format!("Failed to parse regex pattern: {}", e))?;
...
fn ast_to_recexpr_impl(
    ast: &Ast,
    expr: &mut RecExpr<RegexLang>,
) -> Result<Id, String> {
    ...
    match ast {
        ...
        Ast::Concat(concat) => {
            ...
            let concat_id =
                expr.add(RegexLang::Concat(ids.into_boxed_slice()));
            Ok(concat_id)
        }
        ...
        Ast::Alternation(alt) => {
            ...
            for i in 1..alt.asts.len() {
                ...
                result_id =
                    expr.add(RegexLang::Alternation([result_id, right_id]));
            }
                ...
                RepetitionKind::Range(range) => {
                    ...
                    match range {
                        RepetitionRange::Exactly(n) => {
                            ...
                            let id = expr.add(RegexLang::Repeat([
                                child_id, min_id, max_id,
                            ]));
                            Ok(id)
                        }
...
pub fn extract_patterns(
    ...
    while extracted_exprs.len() < min_patterns && attempts < max_attempts {
        ...
        if seen_patterns.insert(pattern.clone()) {
            eprintln!(
                "  [{}] Extracted unique pattern (random sampling): '{}'",
                extracted_exprs.len(),
                pattern
            );
            extracted_exprs.push(expr);
    ...
    eprintln!(
        "Extracted {} unique patterns after {} attempts",
        extracted_exprs.len(),
        num_nodes + attempts
    );
...
fn recexpr_to_pattern_helper(expr: &RecExpr<RegexLang>, id: Id) -> String {
    ...
    match node {
        ...
        RegexLang::Repeat([child, min, max]) => {
            let child_str = recexpr_to_pattern_helper(expr, *child);
            let min_val =
                if let RegexLang::Num(n) = &expr.as_ref()[usize::from(*min)] {
                    *n
                } else {
                    0
                };
            let max_val =
                if let RegexLang::Num(n) = &expr.as_ref()[usize::from(*max)] {
                    *n
                } else {
                    -1
                };
...
pub fn generate_equivalent_patterns(
    pattern: &str,
    iter_limit: usize,
    min_patterns: usize,
    max_patterns: usize,
    ...
    let mut patterns: Vec<String> = expressions
        .iter()
    ...
    if patterns.len() < min_patterns {
        return Err(format!(
            "Not enough equivalent patterns found: {} < {}",
            patterns.len(),
            min_patterns
        ));
    }
